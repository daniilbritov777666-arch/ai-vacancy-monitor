...
- Если включен `AUTO_DELIVERY_ENABLED`, безопасный результат автоматически отправляется заказчику на Freelancehunt; ручная карточка проверки остается для рискованных или заблокированных случаев.
...
- первый отклик, последующие ответы, подготовка результата и сдача результата выполняются автоматически, если настроены токены и действие проходит safety-фильтр.
...
- `AUTO_MODE` - режим AI-слоя:
  ...
  - `autopilot` - AI готовит файлы, автоматически включает outreach/conversation/reply/execution/delivery-флаги и ведет безопасный заказ по цепочке без ручных подтверждений, если цена не выше лимита.
