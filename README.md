...
- После подготовки результата агент присылает карточку проверки в Telegram; результат отправляется заказчику только по кнопке `Разрешить отправку`.
...
- `outbox/delivery_message.sent.json` - отметка, что результат отправлен заказчику после кнопки `Разрешить отправку`.
...
Сейчас автоотправка первого отклика, чтение входящей переписки, автоотправка безопасных последующих ответов, подготовка рабочих пакетов и AI-пакетов результата поддержаны для Freelancehunt через официальный API. Отправка результата выполняется только после кнопки `Разрешить отправку`; принятие оплаты остается точкой подтверждения.
