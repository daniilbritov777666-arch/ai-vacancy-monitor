...
- Скрипт читает официальный API Freelancehunt, RSS FL.ru и живые страницы разовых проектов Freelance.ru/Pchel.net/Weblancer.
...
- Freelancehunt API используется для поиска, проверки проектов, чтения переписки и статусов сделок. Публичный endpoint создания ставок отключён площадкой; агент сохраняет готовый отклик и не помечает его отправленным.
...
Freelancehunt API поддерживает поиск, существующие треды, сдачу и статусы сделки, но создание новой ставки через публичный API отключено площадкой. Freelance.ru, Pchel.net и Weblancer расширяют поиск; первый отклик отправляется только при явно опубликованном email и рабочем SMTP, а последующая автономная переписка требует IMAP. Биржевые действия без официального API, авторизованного канала или открытого контакта агент не имитирует.
