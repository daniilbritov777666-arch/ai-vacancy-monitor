...
Важно: автоматическая отправка последующих сообщений включается отдельно через `AUTO_REPLY_ENABLED=true` и блокируется safety-фильтром при риск-флагах, паролях, обходах лимитов, накрутках и оплате вне безопасной сделки. AI-пакет результата готовится локально, но отправка результата и принятие оплаты остаются точками подтверждения.
