...
- После генерации результата агент присылает карточку проверки в Telegram. Кнопка `Разрешить отправку` отправляет результат заказчику и переводит заказ в `payment_requested`.
...
Важно: автоматическая отправка последующих сообщений включается отдельно через `AUTO_REPLY_ENABLED=true` и блокируется safety-фильтром при риск-флагах, паролях, обходах лимитов, накрутках и оплате вне безопасной сделки. AI-пакет результата готовится локально; отправка результата идет только после кнопки `Разрешить отправку`, принятие оплаты остается точкой подтверждения.
