...
- Текущий статус в коде: добавлена отправка первого отклика через официальный endpoint `POST /v2/projects/{project_id}/bids` после кнопки `Одобрить отклик`.
- По умолчанию отклик уходит в рублях (`RUB`), срок `2` дня, безопасная сделка `employer`; это настраивается через `FREELANCEHUNT_BID_DAYS` и `FREELANCEHUNT_BID_SAFE_TYPE`.
...
Для заказов с опубликованным email теперь поддержан двусторонний разрешенный канал:

- `SMTP_*` отправляет первый отклик, последующие ответы и сдачу результата;
- `IMAP_*` читает входящие письма заказчика и переносит их в `orders/<order_id>/conversation.md`;
- AI-ответы сохраняются в `outbox/email_reply_<email>.md`, а фактическая отправка фиксируется в `outbox/email_reply_<email>.sent.json`.
...
Каждый проход локального агента пишет:
...
- какие каналы готовы к полному циклу: поиск, автоотклик, переписка, watcher оплаты;
- какие площадки работают только как источник проектов;
- где нужен SMTP, API-токен или подключение РФ-платежей;
- где нужен IMAP для входящих email-ответов;
- какие площадки не используются для автодействий из-за отсутствия официального API или открытого контакта.
