...
- [ ] **Step 1: Write failing package detection tests**
...
- [ ] **Step 3: Implement report types and detection**
...
- [ ] **Step 4: Write failing Docker argv tests**
...
- [ ] **Step 5: Implement `DockerExecutionVerifier`**
...
- [ ] **Step 3: Write failing health report test**
...
- [ ] **Step 4: Implement runtime health**
...
- [ ] **Step 1: Write failing quality integration tests**
...
- [ ] **Step 2: Implement verifier injection and report persistence**
...
- [ ] **Step 3: Integrate repair instructions and terminal statuses**
...
- [ ] **Step 1: Write static setup contract tests**

Проверить, что script использует `set -euo pipefail`, устанавливает Homebrew только при отсутствии `brew`, затем `brew install colima docker`, запускает Colima с CPU/memory/disk limits, строит локальный Python runner и inspect-ит оба образа. Dockerfile использует Python 3.12 slim, непривилегированного пользователя и закрепленный pytest.

- [ ] **Step 2: Implement setup assets**
...
- [ ] **Step 3: Run setup contract tests**
...
- [ ] **Step 1: Run safe integration smoke tests**

Во временных папках, не связанных с `orders/`, проверить valid Python, syntax error, timeout, запись в `/workspace` и сетевой запрос. Ожидать passed только для valid; остальные получают соответствующие failed issues. Не использовать ключи или реальные клиентские файлы.

- [ ] **Step 2: Document configuration and operation**
